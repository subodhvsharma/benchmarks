...
/* This has an erroneous use of MPI_ANY_SOURCE in manager() */
...
double manager(double f(double)) {
  ...
  for (i=1; i<nprocs; i++) {
    MPI_Recv(&result, 1, MPI_DOUBLE, MPI_ANY_SOURCE, RESULT_TAG,
	     MPI_COMM_WORLD, &status);  /* any source OK here? svs - Can cause a deadlock
					 but in this example it is OK. 
					 The deadlock can happen if there is another message 
					the say P1 to P0 after P1 has received termination*/
...
int main(int argc, char *argv[]) {
  ...
  MPI_Init(&argc, &argv);
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  //  result = integral(0, pi, .01, f1);
  //if (rank == 0) { printf("%4.20lf\n", result); }
    result = integral(0, pi, .01, f2);
  if (rank == 0) { printf("%4.20lf\n", result); }
  MPI_Finalize();
